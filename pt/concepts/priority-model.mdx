...
- Os **carregadores** não têm valor de recurso — fora de uma janela ativa, o
  conector não está bloqueado por prioridade e é gerido pela gestão de carga
  normal.

## Aplicação/reversão automática

Os perfis são aplicados ou revertidos automaticamente no respetivo
`startTime`/`endTime` — **não são necessárias mais chamadas à API** após a
